<!-- https://github.com/ElemeFE/mint-ui/blob/master/packages/swipe/src/swipe.vue -->
<template>
  <div class="rjcv-swiper">
    <div class="rjcv-swiper-wrap">
      <slot></slot>
      <div class="rjcv-swiper-indicators" v-show="showIndicators && pages.length > 1">
        <div class="rjcv-swiper-indicator" v-for="(page, $index) in pages" :class="{ 'rjcv-swiper-hover': $index === index }"></div>
      </div>
    </div>
  </div>
</template>
...
<script>
import { once, addClass, removeClass } from '../../__assets/js/dom'
  ...
  methods: {
    ...
    doAnimate (towards, options) {
      ...
      var callback = () => {
        if (newIndex !== undefined) {
          ...
          removeClass(oldPage, 'rjcv-swiper-hover')
          addClass(newPage, 'rjcv-swiper-hover')
          this.index = newIndex
        }
        if (this.isDone) {
          this.end()
    ...
    reInitPages () {
      ...
      children.forEach((child, index) => {
        pages.push(child.$el)
        removeClass(child.$el, 'rjcv-swiper-hover')
        if (index === defaultIndex) {
          addClass(child.$el, 'rjcv-swiper-hover')
        }
      })
      this.pages = pages
    },
